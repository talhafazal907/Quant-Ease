...
#    def run_stoch_rsi(self, user_data: dict, data_array: np.ndarray) -> dict:
    ...
    def run_supertrend(self, user_data: dict, data_array: np.ndarray) -> dict:
        """ This function is responsible to run the backtest of the Supertrend Strategy """
        if user_data["market_type"] == "spot":
            @njit
            def loop(data_array: np.ndarray, capital: float, risk: float, reward: float):
                trades = 0
                ...
                # Indices: Open(0), High(1), Low(2), Close(3), Vol(4), Supertrend(5), Trend_Dir(6)
                open_i = 0; high_i = 1; low_i = 2; close_i = 3; dir_i = 6
                
                position_size = 0.0
                entry_price = 0.0
                
                for i in range(1, len(data_array)-1):
                    curr_dir = data_array[i, dir_i]
                    prev_dir = data_array[i-1, dir_i]
                    
                    execution_price = data_array[i+1, open_i]
                    current_high = data_array[i, high_i]
                    current_low = data_array[i, low_i]
                    
                    # Long Signal: Trend_Dir flips from -1 (Bearish) to 1 (Bullish)
                    long_signal = (curr_dir == 1) and (prev_dir == -1)
            ...
            data_array = data_array.astype(np.float64)
            final_cap, wins, losses, max_dd, eq_curve = loop(
                data_array, user_data["capital"], user_data["risk"], user_data["reward"]
            )
            trades = wins + losses
            winrate = wins / trades if trades > 0 else 0.0
            return {
                "final_capital": final_cap, "trades": trades, "win_rate": winrate * 100,
                "max_drawdown": max_dd * 100, "equity_curve": eq_curve, 
                "wins": wins, "losses": losses, "status": 1
            }                
        ...
        if user_data["market_type"] == "futures":
            ...
            def loop(data_array: np.ndarray, capital: float, risk: float, reward: float):
                ...
                for i in range(1, len(data_array)-1):
                    ...
                    # Futures Market Signals
                    long_signal = (curr_dir == 1) and (prev_dir == -1)
                    short_signal = (curr_dir == -1) and (prev_dir == 1)
    ...
    def run(self) -> dict:
        try:
            if self.user_data["strategy_name"] == "double-ema":
                ...
                return results
            elif self.user_data["strategy_name"] == "super_trend":
                data = dl.Load_data(self.user_data["symbol"], self.user_data["time_frame"])
                if len(data) == 0:
                    return {"status": "No data available", "status": 0}
                data_ = i.add_indicator(self.user_data, data)
                if isinstance(data_, dict):
                    return data_
                results = self.run_supertrend(user_data=self.user_data, data_array= data_)
                return results
